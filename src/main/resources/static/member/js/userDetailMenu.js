...
if(location.href.match(/exit/)) {
  ...
  const ordersLiTag = document.querySelector(".link-order");
  currentLocation.innerHTML = ordersLiTag.firstChild.textContent;
} else if(location.href.match(/coupon-generation-issues/)){
  const ordersLiTag = document.querySelector(".link-coupon");
  currentLocation.innerHTML = ordersLiTag.firstChild.textContent;
} else if(location.href.match(/address-list/)){
